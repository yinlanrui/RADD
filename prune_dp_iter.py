...
from pyvacy import analysis, optim, sampling
...
def main(args):
    ...
    name = f'{args.dataset_name}_{args.model_name}_{args.pruner_name}_{args.prune_sparsity}_{args.defend}'
    with open(f'log/{name}.txt', 'a') as appender:
    ...
    return victim_acc, np.mean(shadow_acc_list), np.mean(shadow_prune_acc_list)
